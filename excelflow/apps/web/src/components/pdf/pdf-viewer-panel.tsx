...
export function PdfViewerPanel() {
  ...
  useEffect(() => {
    ...
    void loadPdfBlob(activeSessionId);
    setViewMode('pdf');
  }, [activeSessionId, pdfVersion, loadPdfBlob]);
  ...
  if (!activeSession || !activeSessionId) {
    return (
      <div className="flex h-full flex-col items-center justify-center bg-gray-100 text-gray-400">
        ...
        <p className="mt-1 text-xs">Upload a PDF to view it here</p>
      </div>
    );
  }
  ...
  const handleSaveAndRegenerate = async () => {
    ...
    await regeneratePdf();
    await usePdfStore.getState().selectSession(activeSessionId);
    setViewMode('pdf');
  };
  ...
  const applyReplacements = async () => {
    ...
    try {
      await api.replaceTextInPdf(
        activeSessionId,
        valid.map((row) => ({ find: row.find, replace: row.replace })),
      );
      await loadPdfBlob(activeSessionId);
      await usePdfStore.getState().selectSession(activeSessionId);
      setStatusMessage('PDF updated.');
      setTimeout(() => setStatusMessage(null), 4000);
    } catch (err) {
        ...
        {viewMode === 'pdf' && (
          <div className="h-full w-full bg-gray-200">
            {loadingPdf ? (
              ...
              <iframe src={pdfBlobUrl} className="h-full w-full border-0" title="PDF Preview" />
            ) : (
              <div className="flex h-full flex-col items-center justify-center text-gray-400">
            ...
            <div className="mx-auto max-w-3xl p-6">
              <div className="mb-3 flex items-center justify-between rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
                <p className="text-xs text-amber-700">
                  <strong>Edit Mode</strong> - Changes are applied using style-preserving PDF regeneration.
                </p>
                <button
          ...
          <div className="h-full overflow-y-auto">
            <div className="mx-auto max-w-2xl space-y-4 p-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-semibold text-gray-700">Find & Replace in PDF</h3>
              </div>
              <p className="text-sm text-gray-500">
                This updates text directly in the existing PDF to preserve the original style and layout.
              </p>
              ...
              <div className="mt-6 rounded-lg border border-blue-200 bg-blue-50 p-4">
                <p className="text-xs text-blue-700">
                  Tip: Find & Replace is best when you want guaranteed style preservation for targeted text edits.
                </p>
              </div>
            </div>
          </div>
