  ...
  return (
    <aside className="flex h-full flex-col border-r border-slate-200 bg-white text-slate-800">
      ...
      <div className="scrollbar-thin flex-1 space-y-1 overflow-y-auto p-2">
        {sessions.length === 0 && (
          <div className="mt-8 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center">
            ...
            <p className="text-sm font-medium text-slate-600">No DOCX files yet</p>
            <p className="mt-1 text-xs text-slate-500">Upload a DOCX to start chatting</p>
          </div>
        )}
      ...
      <div className="border-t border-slate-200 p-3 text-center">
        <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500">DOCX AI Workspace</p>
      </div>
    </aside>
  );
}
